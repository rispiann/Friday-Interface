...
// ... (Komponen AuthForm tidak berubah) ...
...
const LoginPage = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isRegister) {
        ...
        // Panggil fungsi login yang sama
  ...
  // 1. Buat fungsi terpusat untuk menangani login yang berhasil
  const handleSuccessfulLogin = (token: string) => {
    if (token) {
      localStorage.setItem('friday_access_token', token);
      // Tambahkan sedikit jeda agar pengguna bisa melihat status "Success"
      setTimeout(() => {
        navigate("/");
      }, 1000); // Jeda 1 detik
    } else {
      setError("Gagal mendapatkan token login.");
    }
  };
      ...
      <div className="relative w-full max-w-md bg-card/20 border border-glass-border rounded-3xl shadow-2xl shadow-black/30 overflow-hidden p-8">
        ...
        <div className="w-full h-[380px] relative">
          <AnimatePresence initial={false}>
            {!isRegister ? (
              <motion.div key="login" initial={{ opacity: 0, x: -100 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 100 }} transition={{ duration: 0.4 }} className="absolute w-full h-full">
              ...
              <motion.div key="register" initial={{ opacity: 0, x: 100 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -100 }} transition={{ duration: 0.4 }} className="absolute w-full h-full">
        ...
        <div className="text-center text-sm">
          ...
          <button onClick={() => { setIsRegister(!isRegister); setError(''); }} className="font-semibold text-primary hover:underline">{isRegister ? "Masuk di sini" : "Daftar sekarang"}</button>
        </div>

        <div className="relative my-6">
          <div className="absolute inset-0 flex items-center"><span className="w-full border-t border-border" /></div>
          <div className="relative flex justify-center text-xs uppercase"><span className="bg-card/20 px-2 text-muted-foreground backdrop-blur-sm">Atau</span></div>
        </div>

        {/* 2. Teruskan fungsi handleSuccessfulLogin sebagai prop onLoginSuccess */}
        <FaceLogin onLoginSuccess={handleSuccessfulLogin} />
